<template>
  <div class="screenwide">
    <q-table
      :wrap-cells="true"
      class="screenwide"
      title="Personell with Notice to Explain"
      :rows="tablerows"
      :columns="columns"
      row-key="name"
      dense
      ...
      <template v-slot:body-cell-actions="props">
        <q-td :props="props">
          <q-btn
            ...
            @click="
              editInfraction(props.row.id), (updateTitle = 'Update NTE Status')
            "
            color="warning"
            size="sm"
          >
    ...
    <q-dialog v-model="edit" persistent>
      <q-card>
        ...
        <q-card-actions align="right">
          <q-btn
            label="Update to Written Explanation"
            color="primary"
            @click="updateInfraction(5, 'we')"
            v-close-popup
          />
          <q-btn
            label="Update NTE Status"
            color="positive"
            @click="updateInfraction(4, 'nte_received')"
            v-close-popup
          />
        </q-card-actions>
      </q-card>
    ...
    const list = {
      ...
      date_received:
        r.nte_received != "1899-11-30"
          ? date.formatDate(r.nte_received, "MMMM DD, YYYY")
          : "",
